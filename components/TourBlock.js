...
export default function TourBlock({tours, pp}) {
    ...
    return <Link href={`/tour/${tours.id}`}><a className="hover:shadow-2xl shadow-dark-blue">
        <div className="w-fit grid grid-cols-1 items-center border border-gray-400 rounded-md text-dark-blue">
            <Image src={tours.images[1].url} width={size.width } height={setHeight()} alt="cappadocia balloon flight"
            ...
            <div className="px-2 my-2">
                ...
                <ul className="text-sm">
                    ...
                    <li className="my-1 flex items-center gap-1">
                        <p className="bg-main-text w-fit p-1 rounded-md text-white font-bold popular">{tours.popular}</p>
                        <p className="bg-yellow-300 w-fit p-1 rounded-md text-white font-bold rated">{tours.rated}</p>
                    </li>
                </ul>
